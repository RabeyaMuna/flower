...
def aggregate_krum(
    ...
    if to_keep > 0:
        ...
        sorted_indices = np.argsort(scores)[::-1]  # type: ignore[index]
        best_indices = sorted_indices[len(scores) - to_keep :].tolist()  # type: ignore[index]
        best_results = [results[i] for i in best_indices]
        return aggregate(best_results)

    # Return the model parameters that minimize the score (Krum)
    min_idx = np.argmin(scores)
    return weights[min_idx]  # type: ignore[index]
...
def _compute_distances(weights: list[NDArrays]) -> NDArray:
    ...
    """
    flat_w: NDArray = np.array([np.concatenate(p, axis=None).ravel() for p in weights])  # type: ignore[assignment, attr-defined]
