...
# Custom type alias for dtype - replacing numpy.typing.DTypeLike which is not valid
# without the numpy mypy plugin in numpy 2.x
DType = np.dtype[Any]


def factor_combine(factor: int, parameters: list[NDArray]) -> list[NDArray]:
    """Combine factor with parameters."""
    return [np.array([factor])] + parameters  # type: ignore[return-value]
...
def factor_extract(
    ...
    return int(parameters[0][0]), parameters[1:]  # type: ignore[return-value]


def get_parameters_shape(parameters: list[NDArray]) -> list[tuple[int, ...]]:
    """Get dimensions of each NDArray in parameters."""
    return [arr.shape for arr in parameters]  # type: ignore[misc]


def get_zero_parameters(
    dimensions_list: list[tuple[int, ...]], dtype: DType = np.int64  # type: ignore[assignment]
) -> list[NDArray]:
