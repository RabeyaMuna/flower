...
1. **Function-specific mods**: These mods apply only to a specific function (e.g, the
...
1. **Function-specific mods** (executed after application-wide mods, in the order they
   are provided)
1. **ClientApp** (core function that handles the incoming `Message` and returns the
   outgoing `Message`)
1. **Function-specific mods** (on the way back, in reverse order)
1. **Application-wide mods** (on the way back, in reverse order)
