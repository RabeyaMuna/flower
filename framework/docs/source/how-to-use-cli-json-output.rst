...
In this guide, we'll show you how to specify a JSON output with the `flwr run`, `flwr ls`, and `flwr stop` commands. We will also provide examples of the JSON output for
...
The |flwr_ls|\_ command lists all the runs in the current project. Similar to `flwr run`, if the app path argument is not passed to `flwr ls`, the current working
...
of `false` and `error-message`, as shown in :ref:`the above example <json_error_output>`. Note that the content of the error message will be different
